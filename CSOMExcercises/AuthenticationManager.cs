...
namespace CSOMExcercises
{
    public class AuthenticationManager : IDisposable
    {
        ...
        private const string defaultAADAppId = "cbefa99a-5ed9-4f83-b2af-b40d5f785fff";
        ...
        public ClientContext GetContext(SharePointConfiguration configuration)
        {
            var web = new Uri(configuration.SiteUri);
            var context = new ClientContext(web);
            ...
            context.ExecutingWebRequest += (sender, e) =>
            {
                string accessToken = EnsureAccessTokenAsync(
                        ...
                        configuration.Email,
                        new System.Net.NetworkCredential(string.Empty, configuration.Password).Password
                     )
                    .GetAwaiter()
                    .GetResult();
                e.WebRequestExecutor.RequestHeaders["Authorization"] = "Bearer " + accessToken;
